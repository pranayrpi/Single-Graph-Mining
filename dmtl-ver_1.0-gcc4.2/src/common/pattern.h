...
RMP_T& random_rmost_path(vector<int, ALLOC<int> > rmp)  {
//vector before shuffling
/*cout<<"Vector before shuffling is "<<endl;
for(int i=0;i<rmp.size();i++)
{
	cout<<"The element at i "<<rmp[i]<<endl;
}*/
random_shuffle(rmp.begin(),rmp.end());
/*cout<<"Vector after shuffling is "<<endl;
for(int i=0;i<rmp.size();i++)
{
	cout<<"The element at i "<<rmp[i]<<endl;
}*/
...
//Jan 31st : get the can code string

string get_can_string()
{
	return _canonical_code.can_string();
} 
