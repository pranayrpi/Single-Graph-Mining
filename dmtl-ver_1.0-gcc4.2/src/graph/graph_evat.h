...
class evat
{
 public:
  ...
  typedef pair<VID_PAIR, int> VID_PAIR_COUNT; /**< Vertex ids pair and the count of it in the current graph*/
  typedef vector<VID_PAIR, ALLOC_<VID_PAIR> > EVAT; /*< Each edge could occur multiple times in 
                                                         a graph */
	int count; //Instead of storing the vat for multiple times store the count
  ...
  unsigned long int byte_size() const{
	...
	bool has_vids_incr_set(int a, int b)
	{
		//increases the count if already present o/w setup the counter
		map<VID_PAIR,int>::iterator it;
