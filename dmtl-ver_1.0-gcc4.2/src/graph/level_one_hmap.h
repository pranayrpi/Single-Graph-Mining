...
public:
  ...
  bool has_label(const V_T& src, const V_T& dest) const {
  ...
  bool has_neighbor(const V_T& src) const {
	  CONST_IT it=_hmap.find(V_EP::conv_hash_type(src));
	  if(it==_hmap.end()) {
		  //cout<<"level_one_map.get_neighbors: src not found in hmap for src="<<src<<endl;
		  //exit(0);
		  //There are no neighbors starting from this vertex...return -1
		  return false;
	  }
	  return true;
  }//end has_neighbors()
  ...
  const NEIGHBORS& get_rand_neighbors(const V_T& src) const {
    CONST_IT it=_hmap.find(V_EP::conv_hash_type(src));
    if(it==_hmap.end()) {
      //cout<<"level_one_map.get_neighbors: src not found in hmap for src="<<src<<endl;
      //exit(0);
...
//Randomize the neighbors
	return std::random_shuffle( it->second.begin(), it->second.end() );
  }//end get_neighbors()
