...
//Jan 31...a new function called can_string which is the string obtained by joining the string corresponding to each tuple
string can_string()
{
	//Generate the can string corresponding to this pattern..
	ostringstream t_ss;
	string ret ="";
	string temp="";
	for(int i=0;i<_dfs_code.size();i++)
	{
		t_ss << _dfs_code[i];
		temp = t_ss.str();
		ret.append(temp);
