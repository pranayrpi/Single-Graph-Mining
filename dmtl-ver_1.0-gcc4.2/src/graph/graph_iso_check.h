...
//	cout<<"Before startup"<<endl;
  iso_startup(cand_pat, src_v, dest_v, e, ids);
//	cout<<"After startup"<<endl;
//	cout<<"New_codes size is "<<ids.size()<<endl;
  ...
  //Feb7 : Pranay...dont add duplicates
vector<typename GRAPH_PATTERN::CAN_CODE::FIVE_TUPLE> fts;
typename GRAPH_PATTERN::CAN_CODE::FIVE_TUPLE ft;
//	cout<<"Codes push start"<<endl;  
	tt_rest.start();
	for(unsigned int i=0; i<ids.size(); i++) {
		ft = typename CAN_CODE::FIVE_TUPLE(0, 1, cand_pat->label(ids[i].first), e, cand_pat->label(ids[i].second));
//check if the tuple is already present
	bool flag = false; //false --> not present already
...
//cout<<"level one start "<<endl;
  ...
  tt_rest.stop();
//cout<<"level one end "<<endl;

  for(unsigned int i=0; i<new_codes.size(); i++) {
//	cout<<"minimal "<<i<<" start"<<endl;
    tt_minimal.start();
    int r = minimal(cand_pat, new_codes[i]);
    if(r==0) {
      cand_pat->_is_canonical=false;
      tt_iso_chk.stop();
      return false;
 ...
 bool has_pair(vector<pair<int,int> > ids,int a,int b)
        {
                int first,second;
...
//Feb7 : Pranay...dont add duplicate new codes...check before adding
  num_startup++;  
  ...
  for(it=cand_pat->begin(); it!=cand_pat->end(); it++) {
	...
	if(!has_pair(ids,it->id, eit_p.first->first))
...
//Feb7:Pranay..check if the vid is already present in new_vids

bool has_int(vector<int> vids,int vi)
{
...
//Feb7:Pranay..returns -1 if finished..continous break if you see -1
template<typename PATTERN, typename CAN_CODE>
int minimal(const PATTERN* cand_pat, CAN_CODE& new_code) {
//cout<<"Entered minimal and new_code size is "<<new_code.size()<<"and cand_pat size is "<<cand_pat->canonical_code().size()<<endl;
...
map<int,int> vids_added;
map<int,int>::iterator vids_it;
  ...
  while(rmp_it>=rmp.begin()) {
    ...
    while(eit_p.first!=eit_p.second) {
      ...
      if(eit_p.first->second<=e) {
		...
		int x = eit_p.first->first;
/*cout<<"Map is "<<endl;
	for ( vids_it=vids_added.begin() ; vids_it != vids_added.end(); vids_it++ )
    cout << (*vids_it).first << " => " << (*vids_it).second << endl;

	  vids_it = vids_added.find(x);
	  if(vids_it==vids_added.end())
	  {
		cout<<"added"<<endl;
		  new_vids.push_back(x);
		  vids_added[x] = 1;
	  }
	  else
	  {
		  cout<<"repetition detected "<<eit_p.first->first<<endl;
	  }*/
		  new_vids.push_back(x);
	}
      }
...
//	  cout<<"New forward vid is "<<new_vids[i]<<endl;
	...
	int ret = minimal(cand_pat, next_code);
